jQuery(function ($) {
    ...
    .on('actionclicked.fu.wizard', function (e, info) {
        if (info.step == 1) {
            if (!$('#Descuentos-form').valid()) {
                e.preventDefault();
            }
            else {

                $("#idRegistro").html($("#idDescConcepto").val().trim())
                $("#totaladeudo").html($("#totalAdeudo").val().trim())
                $("#empresa").html($("#idEmpresa").val().trim())
                ...
                $("#status").html($('#status').val())
                $("#vigenciaindefinida").html($("#vigenciaindefinida").val())
            }
        }
    })
